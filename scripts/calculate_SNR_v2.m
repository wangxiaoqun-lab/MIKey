function [mean_sSNR, sSNR, mean_tSNR, tSNR_map, noise_std] = calculate_SNR_v2(fMRI_data,head_vol, varargin)
...
%   head_vol    - Filename of the head mask NIfTI file
% Optional Name-Value Pairs:
%   percent       - Proportion of noise voxels to use as background (default: 0.005)
%   compute_sSNR  - Flag to enable sSNR calculation (default: true)
...
%   Name - Filename for saving histogram and nii
...
parse(p, fMRI_data, head_vol, varargin{:});

% Extract parameters
percent = p.Results.percent;
compute_sSNR = p.Results.compute_sSNR;
...
head_mask = spm_read_vols(head_vol);
...
if compute_sSNR
    ...
    mean_sSNR = NaN;
    sSNR = NaN(1, size(fMRI_data, 4));
    noise_std = NaN(1, size(fMRI_data, 4));
    sSNR_map = NaN(size(head_mask));
end
...
%% Save files
save_nii_files(sSNR_map, tSNR_map, head_vol,Name, save_path);

%% Visualization
if plotHistogram && ~isnan(mean_tSNR)
    histogramName=[Name,'_tSNRdistribution.jpg'];
    visualize_tSNR(tSNR_map, head_mask, histogramName,save_path);
end
%% Visualization
if plotHistogram && ~isnan(mean_sSNR)
    histogramName=[Name,'_sSNRdistribution.jpg'];
    h = histogram(mean_sSNR, 'BinMethod','fd', 'FaceColor',[0.4 0.6 0.8]);
    xlabel('sSNR'), ylabel('Frequency'), title('sSNR Distribution')
    grid on, box off
    hold on;
    med_val = median(mean_sSNR);
    yl = ylim;
    plot([med_val med_val], yl, 'r--', 'LineWidth',1.5);
    text(med_val+0.1*diff(xlim), yl(2)*0.9,...
        sprintf('Median: %.1f', med_val),...
        'Color','r', 'FontSize',10);
    saveas(fig,  histogramName);
    close(fig);
end
...
function save_nii_files(sSNR_map, tSNR_map, head_vol,Name, save_path)
% Save sSNR map
if ~all(isnan(sSNR_map(:)))
ssnr_vol = head_vol;
ssnr_vol.fname = fullfile(save_path,[Name, '_sSNRmap.nii']);
...
if ~all(isnan(tSNR_map(:)))
    tsnr_vol = head_vol;
    tsnr_vol.fname = fullfile(save_path,[Name '_tSNRmap.nii']);
...
function visualize_tSNR(tSNR_map, mask, fname,save_path)
...
% Save figure
saveas(fig, fullfile(save_path,fname));
close(fig);
